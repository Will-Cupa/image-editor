...
Image Image::luminosityUp(float facteur) const {
  ...
  if (facteur < 1) { //Verification du facteur
    throw invalid_argument("La luminosité doit être supérieure à 1"); //Envoie d'une erreur en cas de facteur invalide
  }
  for (int i = 0; i < _hauteur; i++) {
    for (int j = 0; j < _largeur; j++) {
      if (I_LumUp._rouge[i][j] == 0) I_LumUp._rouge[i][j] == 1; //Pour éviter une multiplication par 0
      if (I_LumUp._rouge[i][j] == 0) I_LumUp._rouge[i][j] == 1;
      if (I_LumUp._rouge[i][j] == 0) I_LumUp._rouge[i][j] == 1;
...
Image Image::luminosityDown(float facteur) const {
  ...
  if (facteur > 1) { //Verification du facteur
    throw invalid_argument("La luminosité doit être inférieure à 1"); //Envoie d'une erreur en cas de facteur invalide
  }
  for (int i = 0; i < _hauteur; i++) {
    for (int j = 0; j < _largeur; j++) {
...
Image Image::contrastUp(float facteur) const {
  ...
  for (int i = 0; i < _hauteur; i++) {
    for (int j = 0; j < _largeur; j++) {
      I_ContrastUp._rouge[i][j] =
          (I_ContrastUp._rouge[i][j] - 128) * facteur + 128;
...
Image Image::reglageAuto() const {
  ...
  if(avgLum == 0) avgLum = 1; // Eviter une division par 0
...
Filtre GRADIENTX(vector<vector<float>>{{-1,0,1},
                                       {-2,0,2},
                                       {1,0,1}});

Filtre GRADIENTY(vector<vector<float>>{{-1,-2,1},
                                       {0,0,0},
                                       {1,2,1}});
...
bool Image::operator== (const Image& I1) const{
  for (int i = 0; i < _hauteur; i++) {
    for (int j = 0; j < _largeur; j++) {
      if(_rouge[i][j] != I1._rouge[i][j] || _vert[i][j] != I1._vert[i][j] || _bleu[i][j] != I1._bleu[i][j]){
        return false;
      }
    }
  }
